...
const { summarizeRelease, pickLatestDmgAsset } = require('./release');
const { makeOutputName } = require('./build');
...
function createLocalBuilder() {
  const builderEntry = path.resolve(__dirname, '../../index.js');
...
function getDefaultDeps(overrides = {}) {
  ...
  const githubRepo = env.CI_RELEASE_REPO || env.GITHUB_REPOSITORY || '';
  ...
  return {
    releaseApi: overrides.releaseApi || {
      async getLatest() {
        if (!githubRepo) {
          throw new Error('Missing repository for latest release lookup (set CI_RELEASE_REPO or GITHUB_REPOSITORY)');
        }
...
async function runCacheMode(deps) {
  ...
  const locationInput = await deps.io.prompt('Download location', defaultLocation);
...
async function runBuildMode(parsed, deps) {
  ...
  const outputName = makeOutputName(buildVersion);
